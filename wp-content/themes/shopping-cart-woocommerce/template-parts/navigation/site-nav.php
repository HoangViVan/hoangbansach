...
<?php
$shopping_cart_woocommerce_sticky = get_theme_mod('shopping_cart_woocommerce_sticky');
    $shopping_cart_woocommerce_data_sticky = "false";
    if ($shopping_cart_woocommerce_sticky) {
    $shopping_cart_woocommerce_data_sticky = "true";
    }
    global $wp_customize;
?>

<div class="innermenuboxupper <?php if( is_user_logged_in() && !isset( $wp_customize ) ){ echo "login-user";} ?>" data-sticky="<?php echo esc_attr($shopping_cart_woocommerce_data_sticky); ?>">
	<div class="container">
		<div class="innermenubox">
	        ...
	        if($shopping_cart_woocommerce_logo_settings == 'Different Line'){ ?>
	          <div class="logo mb-md-0 text-center text-lg-left">
	            ...
	            <?php if(get_theme_mod('shopping_cart_woocommerce_site_title',true) == 1){ ?>
	              <h1><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></h1>
	            <?php }?>
	            <?php $description = get_bloginfo( 'description', 'display' );
	            if ( $description || is_customize_preview() ) : ?>
	              <?php if(get_theme_mod('shopping_cart_woocommerce_site_tagline',false)){ ?>
	        ...
	        <?php }?>
				...
				<div class="col-lg-2 col-md-4 col-sm-7 col-9 search-box align-self-center text-right">
					<?php if(get_theme_mod('shopping_cart_woocommerce_search_icon',true) != ''){ ?>
          	<button class="search_btn"><i class="fas fa-search"></i></button>
        	<?php }?>
					<?php if(get_theme_mod('shopping_cart_woocommerce_user_icon',true) != ''){ ?>
						...
						<?php } ?>
					<?php }?>
					<?php if(get_theme_mod('shopping_cart_woocommerce_cart_icon',true) != ''){ ?>
						<?php if(class_exists('woocommerce')){ ?>
							<span><a href="<?php if(function_exists('wc_get_cart_url')){ echo esc_url(wc_get_cart_url()); } ?>" title="<?php esc_attr_e( 'shopping cart','shopping-cart-woocommerce' ); ?>"><i class="fas fa-shopping-basket"></i></a></span>
